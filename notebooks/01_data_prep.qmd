...
```{r}
# can remove sediment, but should be careful about indices after
...
pts3 <- here('data/raw/1-s2.0-S0031018220302947-mmc1.xlsx') |>
  ...
  mutate(Latitude = if_else(Country %in% c('Peru', 'Chile') & Latitude > 0, 
                            Latitude * -1, Latitude)) |> # some human samples have the wrong hemisphere
...
other_predictors[['sediment']]  <- as.int(other_predictors[['sediment']]) # not clear why this is necessary, but it is
