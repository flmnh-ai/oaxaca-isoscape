...
#### Bataille et al. (2020) Isotope Predictors
...
```{r}
vars1 <- rast(pred_files[c(4, 15, 18)])
names(vars1) <- pred_names[c(4, 15, 18)]
eck4 <- st_crs(crs(vars1))
#vars1[[1]] <- as.factor(vars1[[1]])

vars2 <- rast(pred_files[c(1,2,3,20,21)])
crs(vars2) <- crs(vars1)
names(vars2) <- pred_names[c(1,2,3,20,21)]

#vars3 <- rast(pred_files[23]) |>
# project(vars1, method = 'near') |>
#  as.factor()
#names(vars3) <- 'sediment_type'

predictors <- c(vars1, vars2)#, vars3)
bedrock_predictors <- predictors[[3:8]]
other_predictors <- predictors[[1:2]]
...
#### Other Global Predictors

Get other global predictors from the geodata package (elevation and soils) and the CHELSA climate dataset.
...
Aerosols deposition from MERRA2
...
mexico_bedrock <- crop(bedrock_predictors, mexico_bbox) |>
  fillNA(method = 'modal') |>
  predict(bedrock_pcs, index = 1:3, cores = 4)
...
mexico_soils <- crop(soils, st_transform(mexico_bbox, crs(soils))) |> 
  predict(soil_pcs, index = 1:5, cores = 4) |>
  project(mexico_other) |>
  fillNA('mean')
...
#mapview(oaxaca_plants |> st_jitter(500), zcol = 'Sr')
...
```{r}
saveRDS(pts_combined, 'pts_combined.rds')
```

```{r}
mapview(pts_combined, zcol = 'type')
```
...
test_fun <- function(x, rast, crs = 4326){
...
climate_extract <- map_dfc(1:nlyr(climate), test_fun, climate) %>%
  predict(climate_pcs, .) %>%
  .[,1:7] %>%
  as_tibble() |> 
  setNames(climate_pc_names)

soils_extract <- map_dfc(1:nlyr(soils), test_fun, soils, crs(soils)) %>%
  predict(soil_pcs, .) %>%
  .[,1:5] |>
  as_tibble() |> 
...
bedrock_extract <- map_dfc(1:nlyr(bedrock_predictors), test_fun, bedrock_predictors, eck4) %>%
  predict(bedrock_pcs, .) %>%
  .[,1:3] |>
  as_tibble() |>
  setNames(bedrock_pc_names)

other_extract <-  map_dfc(1:nlyr(other_predictors), test_fun, other_predictors, eck4) 
```
